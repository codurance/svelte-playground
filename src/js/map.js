  ...
  d3.json(FINAL)
    .then(barcelona => {
      const features = topojson.feature(
        barcelona,
        barcelona.objects.ABS_2018,
      ).features;

      const tooltipValues = barcelona.objects.ABS_2018.geometries.map(
        ({ properties }) =>
          properties.VALORES ? properties.VALORES[indexOfFilter] : 0,
      );
      const quantize = d3
        .scaleQuantize()
        ...
        tooltip
          .html(
            d.properties.NOMABS +
              ...
              d.properties.NOMSS +
              '<br />' +
              d.properties.VALORES[indexOfFilter],
          )
      ...
      svg
        ...
        .text(({ properties }) =>
          properties.NOMABS.replace('Barcelona - ', ''),
        );
    })
